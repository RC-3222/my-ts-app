...
export const AppEternal = () => {
    ...
    const loadMoreItems = useCallback(async () => {
        if (data?.items && data.items.length > 0 && !data?.next) return;

        setIsLoading(true);

        try {
            const resp = await fetch((data?.items && data.items.length > 0 ? data.next as string : `https://rickandmortyapi.com/api/character`));
    ...
    return <>
        {data?.items && data.items.length > 0 && <div className={styles.cardGrid}>
